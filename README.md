...
*   Возможность появления дубликатов при повторной публикации. Транзакция – частичное решение, но простое. Двухфазный коммит – костыль на костыле + rabbit его не поддерживает + плохая оптимизация. Решение: использовать паттерн outbox(также решает проблему переполнения буфера).
*   Потеря сообщений при переполнении буфера.2 Решения: 1 способ - Outbox(закрывает сразу 2 проблемы. Недостатки: высокая нагрузка на бд). 2 способ – CDC + лог. репликации.
